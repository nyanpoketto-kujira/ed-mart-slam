...
private:
	...
	// --- FEVER MODE ---
	SDK::Timer FeverTimer{};
	const int FeverThreshold = 20;
	const float FeverDuration = 7.0f;
	SDK::Text FeverText{};

public:
	ED() {
		FeverText.Init(SDK::FONTNAME.Main, FW_DONTCARE);
		FeverText.EnableShadow();
		FeverText.SetShadow(0.1, -0.1, 0.7);
	...
	void InteractWithCoffee(SDK::KeyEvent& Event, SDK::Object* Shelf, ItemStruct& Item) {
		...
		if (SDK::GLOBAL.FeverState)
			IsCorrect = true;
		else if (Item.Type == Coffee_Box && Event.Key == VK_DOWN)
			IsCorrect = true;
	...
	void UpdateFrame(float FrameTime) {
		...
		if (SDK::GLOBAL.FeverState) {
			FeverTimer.Update(FrameTime);
			if (FeverTimer.CheckMiliSec(FeverDuration, 1.0, CHECK_AND_RESET)) {
				SDK::GLOBAL.FeverState = false;
				SDK::GLOBAL.FeverMeter = 0;
				SDK::SoundTool.ResetPlaySpeed(SDK::CHANNEL.BGM);
			}
		}
		else if (SDK::GLOBAL.FeverMeter >= FeverThreshold) {
			SDK::GLOBAL.FeverState = true;
			SDK::SoundTool.SetPlaySpeed(SDK::CHANNEL.BGM, 1.2f);
			FeverTimer.Reset();
		}
	...
	void RenderFunc() {
		if (SDK::GLOBAL.FeverState) {
			ShakeTimer.Update(0.016f);
			if (ShakeTimer.CheckMiliSec(0.02, 2, CHECK_AND_INTERPOLATE)) {
				ShakeValue.x = SDK::Random.Gen(RANDOM_TYPE_REAL, -0.02, 0.02);
				ShakeValue.y = SDK::Random.Gen(RANDOM_TYPE_REAL, -0.02, 0.02);
			}
		} else if (Frame != ED_GameOver) {
			ShakeValue.x = 0; ShakeValue.y = 0;
		}
		...
		if (SDK::GLOBAL.FeverState) {
			SDK::ImageTool.SetColorRGB(255, 100, 100); // Red tinted ED
		}
		SDK::ImageTool.RenderStaticSpriteSheet(SDK::IMAGE.ED, Frame + CoffeeStatinLevel * 9);
		if (SDK::GLOBAL.FeverState) {
			SDK::ImageTool.SetColorRGB(255, 255, 255); // Reset tint
		}

		if (SDK::GLOBAL.FeverState) {
			FeverText.SetColorRGB(255, 100, 0); // Orange color for Fever!
			FeverText.SetOpacity(1.0);
			float Remaining = FeverDuration - FeverTimer.GetCurrentTime();
			if (Remaining < 0) Remaining = 0.0f;
			FeverText.Render(0.0, 0.5, 0.2, L"FEVER! %.1f", Remaining);
		}
		else if (SDK::GLOBAL.FeverMeter > 0) {
			FeverText.SetColorRGB(255, 200, 100); // Light orange for pre-fever
			FeverText.SetOpacity(0.8);
			FeverText.Render(0.0, 0.5, 0.1, L"Fever: %d / %d", SDK::GLOBAL.FeverMeter, FeverThreshold);
		}
	}
